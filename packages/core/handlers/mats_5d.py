"""
Обработчик для сценария заказа 5D-ковриков с FSM.
"""
...
@router.callback_query(F.data.in_(["category:mats_5d", "category:5d_mats", "category:5d_mats_deluxe"]))
async def start_mats_5d_order(callback: CallbackQuery, state: FSMContext, config: Config):
    """
    Начало сценария заказа 5D-ковриков.
    Поддерживает различные callback_data для разных конфигураций:
    - category:mats_5d (legacy)
    - category:5d_mats (evopoliki)
    - category:5d_mats_deluxe (5deluxe)
    ...
    category_name = i18n.get(f"buttons.categories.{category_code}") or i18n.get("buttons.categories.5d_mats")

    # Сохраняем категорию в состоянии
    await state.update_data(category="5d_mats", category_name=category_name)

    # Переключаем состояние
    await state.set_state(OrderFlow.waiting_for_brand)

    text = i18n.get("categories.5d_mats.intro") or (
        f"🏆 <b>{category_name}</b>\n\n"
        f"Отлично! Давайте подберем коврики для вашего автомобиля.\n\n"
...
async def process_brand_selection(callback: CallbackQuery, state: FSMContext, config: Config):
    ...
    builder.row(
        InlineKeyboardButton(
            ...
            callback_data="category:5d_mats"
        )
    )
    builder.row(
        InlineKeyboardButton(
...
async def process_suggestion_confirmation(callback: CallbackQuery, state: FSMContext, config: Config):
    ...
    category = data.get("category", "mats_5d")
...
async def process_option_selection(callback: CallbackQuery, state: FSMContext, config: Config):
    ...
    category_name = data.get("category_name", "5D-коврики")
    ...
    if option == "need_consultation":
        ...
        await send_request_to_group(
            callback.bot,
            callback,
            category=f"mats_5d ({brand_name} {model_name}, требуется консультация, config=config)"
        )
