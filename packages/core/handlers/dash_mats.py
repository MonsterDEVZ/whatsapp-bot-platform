"""
Обработчик для сценария заказа Накидок на панель с FSM.
"""
...
@router.callback_query(F.data.in_(["category:dash_mats", "category:dashboard_covers", "category:alcantara_dash"]))
async def start_dash_mats_order(callback: CallbackQuery, state: FSMContext, config: Config):
    """
    Начало сценария заказа Накидок на панель.
    Поддерживает различные callback_data:
    - category:dash_mats (legacy)
    - category:dashboard_covers (evopoliki)
    - category:alcantara_dash (5deluxe)
    ...
    category_name = i18n.get(f"buttons.categories.{category_code}") or i18n.get("buttons.categories.dashboard_covers")

    # Сохраняем категорию в состоянии  
    await state.update_data(category="dashboard_covers", category_name=category_name)

    # Переключаем состояние
    await state.set_state(OrderFlow.waiting_for_brand)

    text = i18n.get("categories.dashboard_covers.intro") or (
        f"🎯 <b>{category_name}</b>\n\n"
        f"Отлично! Давайте подберем накидки для вашего автомобиля.\n\n"
        f"📝 Выберите марку автомобиля:"
    )
...
async def process_brand_selection(callback: CallbackQuery, state: FSMContext, config: Config):
    ...
    Ищет модели для выбранной марки в PostgreSQL и показывает запрос модели.
    """
    ...
    builder.row(
        InlineKeyboardButton(
            ...
            callback_data="category:dashboard_covers"
        )
    )
    builder.row(
        InlineKeyboardButton(
...
async def request_brand_input(callback: CallbackQuery, config: Config):
    ...
    """
    text = (
        "⌨️ Введите марку автомобиля вручную:\n\n"
        "<i>Например: Toyota, BMW, Mercedes</i>"
    )
...
async def process_brand_button(callback: CallbackQuery, state: FSMContext, config: Config):
    ...
    text = (
        f"✅ Марка: <b>{brand_name}</b>\n\n"
        f"📝 <b>Шаг 2/2:</b> Выберите модель автомобиля или введите вручную"
    )
...
async def request_model_input(callback: CallbackQuery, config: Config):
    ...
    """
    text = (
        "⌨️ Введите модель автомобиля вручную:\n\n"
        "<i>Например: Camry, Corolla, X5</i>"
    )
...
async def process_suggestion_confirmation(callback: CallbackQuery, state: FSMContext, config: Config):
    ...
    category = data.get("category", "dash_mats")
    ...
    async for session in get_session():
        ...
        if patterns:
            # Лекала найдены!
            await handle_patterns_found(callback.message, state, patterns, brand_name, model_name, config)
        else:
...
async def handle_patterns_found(message, state: FSMContext, patterns: list, brand_name: str, model_name: str, config):
    """
    Обрабатывает случай, когда лекала найдены в базе.
    Для накидок на панель сразу переходит к оформлению заказа без опций.
    """
    ...
    data = await state.get_data()
    category_name = data.get("category_name", "Накидки на панель")

    # Рассчитываем базовую цену (без дополнительных опций)
    async for session in get_session():
        tenant = await get_tenant_by_slug(session, config.bot.tenant_slug)
        model, body_type = await get_model_with_body_type(session, brand_name, model_name)

        # Для накидок на панель нет дополнительных опций
        selected_options = {}
    ...
    # Показываем итоговую информацию и сразу запрашиваем контакты
    text = (
        f"✅ <b>Отличные новости!</b>\n\n"
        f"У нас есть накидки на панель для <b>{brand_name} {model_name}</b>!\n\n"
        f"📋 <b>Сводка вашего заказа:</b>\n"
        f"🚗 <b>Автомобиль:</b> {brand_name} {model_name}\n"
        f"📦 <b>Товар:</b> {category_name}\n"
        f"💰 <b>Стоимость:</b> {total_price} сом\n\n"
        f"Отлично! Теперь мне нужна ваша контактная информация,\n"
        f"чтобы наш менеджер мог с вами связаться.\n\n"
        f"📝 <b>Шаг 1/2:</b> Введите ваше имя"
    )
    ...
    # Сразу переходим к сбору контактной информации
...
async def handle_patterns_not_found(message, state: FSMContext, brand_name: str, model_name: str):
    ...
    text = (
        ...
        f"создадим накидки специально для вашего автомобиля!\n\n"
...
async def process_model(message: Message, state: FSMContext, config: Config):
    ...
    async for session in get_session():
        ...
        if patterns:
            # Лекала найдены!
            await handle_patterns_found(search_msg, state, patterns, brand_name, model_name, config)
        else:
...
async def process_option_selection(callback: CallbackQuery, state: FSMContext, config: Config):
    ...
    if option == "need_consultation":
        ...
        await send_request_to_group(
            callback.bot,
            callback,
            category=f"dash_mats ({brand_name} {model_name}, требуется консультация, config=config)"
        )
