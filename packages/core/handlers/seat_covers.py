"""
Обработчик для сценария заказа Автомобильных чехлов с FSM.
"""
...
@router.callback_query(F.data.in_(["category:seat_covers", "category:premium_covers"]))
async def start_seat_covers_order(callback: CallbackQuery, state: FSMContext, config: Config):
    """
    Начало сценария заказа Автомобильных чехлов.
    Поддерживает различные callback_data:
    - category:seat_covers (evopoliki)
    - category:premium_covers (5deluxe)
    ...
    category_name = i18n.get(f"buttons.categories.{category_code}") or i18n.get("buttons.categories.seat_covers")

    # Сохраняем категорию в состоянии
    await state.update_data(category="seat_covers", category_name=category_name)

    # Переключаем состояние
    await state.set_state(OrderFlow.waiting_for_brand)

    text = i18n.get("categories.seat_covers.intro") or (
        f"💺 <b>{category_name}</b>\n\n"
        f"Отлично! Давайте подберем чехлы для вашего автомобиля.\n\n"
...
async def process_brand_selection(callback: CallbackQuery, state: FSMContext, config: Config):
    ...
    builder.row(
        InlineKeyboardButton(
            ...
            callback_data="category:seat_covers"
        )
    )
    builder.row(
        InlineKeyboardButton(
...
async def process_suggestion_confirmation(callback: CallbackQuery, state: FSMContext, config: Config):
    ...
    category = data.get("category", "seat_covers")
...
async def handle_patterns_found(message, state: FSMContext, patterns: list, brand_name: str, model_name: str, config):
    ...
    Для чехлов сразу переходит к оформлению заказа без опций.
    """
    ...
    data = await state.get_data()
    category_name = data.get("category_name", "Автомобильные чехлы")
    ...
    async for session in get_session():
        ...
        # Для чехлов нет дополнительных опций
        selected_options = {}
    ...
    text = (
        ...
        f"У нас есть чехлы для <b>{brand_name} {model_name}</b>!\n\n"
...
async def handle_patterns_not_found(message, state: FSMContext, brand_name: str, model_name: str):
    ...
    text = (
        ...
        f"создадим чехлы специально для вашего автомобиля!\n\n"
...
async def process_option_selection(callback: CallbackQuery, state: FSMContext, config: Config):
    ...
    if option == "need_consultation":
        ...
        await send_request_to_group(
            callback.bot,
            callback,
            category=f"seat_covers ({brand_name} {model_name}, требуется консультация, config=config)"
        )
