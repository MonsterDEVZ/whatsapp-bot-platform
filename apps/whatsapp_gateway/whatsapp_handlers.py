...
async def handle_phone_input(chat_id: str, phone: str, config: Config) -> str:
    """
    Обрабатывает ввод номера телефона.
    Отправляет заявку в группу менеджеров.

    Args:
        chat_id: ID чата WhatsApp
        phone: Введённый номер
        config: Конфигурация tenant
    ...
    """
    # Сохраняем телефон
    update_user_data(chat_id, {"client_phone": phone.strip()})
