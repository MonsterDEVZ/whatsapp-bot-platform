    ...
    # Обработка текстового ввода - fuzzy search по всем маркам
        ...
        if exact_match:
            ...
            if matches:
                ...
                # 🔄 HYBRID MODE: Если fuzzy search не нашел совпадений,
                # возвращаем None - возможно, это вопрос для AI
                logger.info(f"🔄 [HYBRID] Марка '{brand_input}' не найдена, возвращаем None для маршрутизации к AI")
                return None
    ...
    # Обработка текстового ввода - поиск по всем моделям
        ...
        if exact_match:
            ...
            if matches:
                ...
                # 🔄 HYBRID MODE: Если fuzzy search не нашел совпадений,
                # возвращаем None - возможно, это вопрос для AI
                logger.info(f"🔄 [HYBRID] Модель '{model_input}' не найдена, возвращаем None для маршрутизации к AI")
                return None
...
async def handle_option_selection(
    ...
    # 🔄 HYBRID MODE: Если команда не распознана, возвращаем None
    # Это сигнал для route_message_by_state передать запрос в AI
    if not option_code:
        return None
